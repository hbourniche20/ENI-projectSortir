 ...
 */
final class Version20211022121903 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, ville_id, email, roles, password, pseudo, prenom, nom, tel FROM user');
        ...
        , password VARCHAR(255) NOT NULL COLLATE BINARY, pseudo VARCHAR(255) NOT NULL COLLATE BINARY, prenom VARCHAR(255) NOT NULL COLLATE BINARY, nom VARCHAR(255) NOT NULL COLLATE BINARY, tel VARCHAR(255) NOT NULL COLLATE BINARY, CONSTRAINT FK_8D93D649A73F0036 FOREIGN KEY (ville_id) REFERENCES ville (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO user (id, ville_id, email, roles, password, pseudo, prenom, nom, tel) SELECT id, ville_id, email, roles, password, pseudo, prenom, nom, tel FROM __temp__user');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('DROP TABLE sortie');
        $this->addSql('CREATE TABLE sortie (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, ville_organisatrice_id INTEGER NOT NULL, ville_accueil_id INTEGER NOT NULL, site_id INTEGER DEFAULT NULL, organisateur_id INTEGER NOT NULL, nom VARCHAR(255) NOT NULL, date_sortie DATETIME NOT NULL, date_limite_inscription DATE NOT NULL, nb_places INTEGER NOT NULL, duree INTEGER NOT NULL, description VARCHAR(255) NOT NULL, publiee BOOLEAN NOT NULL, motif_annulation VARCHAR(255) DEFAULT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO sortie (id, ville_organisatrice_id, ville_accueil_id, site_id, organisateur_id, nom, date_sortie, date_limite_inscription, nb_places, duree, description, publiee, motif_anullation) SELECT id, ville_organisatrice_id, ville_accueil_id, site_id, organisateur_id, nom, date_sortie, date_limite_inscription, nb_places, duree, description, publiee, motif_annulation FROM __temp__sortie');
        $this->addSql('DROP TABLE __temp__sortie');
        $this->addSql('CREATE INDEX IDX_3C3FD3F2B8C8DEB3 ON sortie (ville_organisatrice_id)');
        $this->addSql('CREATE INDEX IDX_3C3FD3F2A2C2D9C8 ON sortie (ville_accueil_id)');
        $this->addSql('CREATE INDEX IDX_3C3FD3F2F6BD1646 ON sortie (site_id)');
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, ville_id, email, roles, password, pseudo, prenom, nom, tel FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, ville_id INTEGER NOT NULL, email VARCHAR(180) NOT NULL, roles CLOB NOT NULL --(DC2Type:json)
        , password VARCHAR(255) NOT NULL, pseudo VARCHAR(255) NOT NULL, prenom VARCHAR(255) NOT NULL, nom VARCHAR(255) NOT NULL, tel VARCHAR(255) NOT NULL)');
        $this->addSql('INSERT INTO user (id, ville_id, email, roles, password, pseudo, prenom, nom, tel) SELECT id, ville_id, email, roles, password, pseudo, prenom, nom, tel FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_8D93D649E7927C74 ON user (email)');
        $this->addSql('CREATE INDEX IDX_8D93D649A73F0036 ON user (ville_id)');
